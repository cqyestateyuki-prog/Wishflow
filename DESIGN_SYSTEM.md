...
| `--shadow` | `0 1px 2px rgba(46,43,51,0.06), 0 6px 16px rgba(46,43,51,0.06)` | 卡片默认中性纸影（利落、极淡） |
| `--shadow-lift` | `0 2px 4px rgba(46,43,51,0.08), 0 10px 24px rgba(46,43,51,0.10)` | 主按钮/hover 抬升投影（同为中性） |
...
**表单可读下限（2026-07 定版）**：需要阅读/操作的正文与标签一律 ≥12px。表单问句标签用 **15px**、选项/档位按钮 **14px**（如 /try 的"这是什么类型的愿望？"及短期/长期、几周/几个月/几年）；连接档位标签 ≥12px；徽章/标签 12px。只有纯装饰性说明（如"点空白处离开 / Tap anywhere outside to leave"）可低于 12px。
...
- **投影**：一律中性墨色极淡投影（`--shadow` / `--shadow-lift`，2026-07-10 定版：紫系大半径柔光在纸白底上显脏、拉低对比，弃用）；双层结构=1 层贴身锐影 + 1 层小半径薄影，模糊半径 ≤24px，y 位移 ≤10px
